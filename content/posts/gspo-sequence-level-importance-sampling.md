...
事实一：**ε 差三个数量级，不是笔误，是平均的必然**。token 级比率单个就能冲到 1.2；序列级比率是逐 token 对数比率的平均再取 exp，平均会把正负涨落抵消掉，$s_i$ 天然贴着 1 小幅波动。这笔账可以算细（以下为推算，论文没有报告这些统计量）：设一次 mini-batch 更新后，平均每个 token 的对数比率漂移标准差为 $\sigma$（比如 5%），回答长 $L$，各 token 漂移近似独立，则平均后的漂移标准差缩到 $\sigma/\sqrt{L}$——$L=2000$ 时约为 $1/45$。给这样一个几乎不动的比率配 $\pm 0.2$ 的裁剪带，它永远也碰不到边，裁剪等于不存在，所以 GSPO 必须收到万分之几：$0.2 / 3\times10^{-4} \approx 667$，三个数量级的收窄就是这么来的。换个说法：3e-4 的容忍带对应平均每个 token 只允许约 0.03% 的对数漂移。还要交代假设的成色：实际训练中各 token 的漂移并不独立（一次更新会成片地抬高或压低相似上下文的概率），真实缩小倍数到不了 $\sqrt{L}$——但这只会让下面事实二的结论更强。

事实二：**被裁的 token 多一百倍，效率反而更高**。带窄了一千倍，被裁的反而多一百倍，听起来矛盾，两笔账乘起来就不矛盾了（以下推算同上）。第一笔：相对各自的波动幅度，带反而更窄了。波动只缩了约 45 倍，带缩了 667 倍，拿各自的波动当尺子量，GSPO 的带比 GRPO 窄约 15 倍（$\sqrt{L}/667$），$s_i$ 出界比 $w_t$ 出界容易得多。代入具体数（$\sigma=5\%$、漂移近似高斯）：GRPO 的 0.2 相当于 $4\sigma$，单个 token 出界概率为万分之几；GSPO 的 3e-4 只相当于约 $0.27\sigma$（$0.0003/(0.05/45)$），每条 response 有过半概率整条出局。第二笔：出界按整条算账。GRPO 一个 token 出界只废它自己；GSPO 出界一次，整段几千 token 全部被剔除。两个因素相乘，被裁 token 总数就是 GRPO 的约一百倍——论文图 2 实测的裁剪比例差正好是约两个数量级，量级对得上。真正反直觉的是后半句：用于训练的 token 更少，训练效率和 benchmark 表现却更好（AIME'24 / LiveCodeBench / CodeForces 三条曲线都赢）。论文的解读：GRPO 的 token 级梯度估计本来就噪声大，被随机保留下来的那部分并不比被裁掉的部分更有用；GSPO 的序列级筛选提供了更可靠的学习信号——少而干净，胜过 多而脏。
...
$$\varepsilon \approx k \cdot \frac{\sigma}{\sqrt{L}}$$

其中 $\sigma$ 是一次更新后逐 token 对数比率的漂移幅度，$L$ 是回答长度，$k$ 是想放过的波动倍数。把前面的演示数（$\sigma=5\%$、$L=2000$）代入，$\sigma/\sqrt{L} \approx 0.0011$，论文的 3e-4 相当于 $k \approx 0.27$——只放过约四分之一的日常波动，是很严的筛选，与实测"裁掉百分之几十 token"互相印证。

这个框架同时说明 3e-4 不能照搬，它和三件事耦合：off-policy 程度（数据越陈旧 $\sigma$ 越大，$\varepsilon$ 要相应放宽，否则样本全被裁光）；回答长度（$L$ 越长平均后波动越小，同样的 $\varepsilon$ 越严，32k 与 2k 的合适取值差约 4 倍）；想要的裁剪力度（最优裁剪比例没有公开数据，复现只能自己扫参）。换一套配置，这个数就要重新定。
...
- $\varepsilon$ 的选取方法本身没有公开：3e-4/4e-4 只有结果没有消融，$\varepsilon \approx k \cdot \sigma/\sqrt{L}$ 只是本文的推断框架，$k$ 的最优区间、与 clip-higher 式不对称边界叠加的效果，都需要公开复现；
- 几何平均的稀释效应在超长 CoT（32k token 以上）上是否会系统性放过"局部剧变"，需要构造性实验验证；
- GSPO-token 在多轮 agentic RL 里与 turn 级奖励配合的实际效果，目前只有 Qwen 内部数据。

---

> **更新记录（2026-08-29）**：据读者三轮问答回流增强——「证据」一节补入裁剪带算术的完整推导（$\sigma/\sqrt{L}$ 方差缩减、667 倍收窄、相对带窄约 15 倍、出界概率演算），新增「附：裁剪范围 3e-4/4e-4 是怎么定的」（论文未披露选取方法；推断框架与三个耦合），未决问题第一条随之改写。所有推演均为标注的推算，非论文原文内容。
